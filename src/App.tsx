...
import { Heart, Music, Send, Share2 } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-secondary overflow-x-hidden selection:bg-primary/20 selection:text-primary">
      {/* Copy Toast */}
      <motion.div
      ...
      <section className="relative h-screen flex flex-col items-center justify-center text-center px-6 overflow-hidden">
        ...
        <div className="relative z-10 space-y-8">
          <motion.div
            ...
            className="space-y-4"
          >
            <span className="text-white/80 uppercase tracking-[0.4em] text-xs font-medium">
              The Wedding of
            </span>
            <h1 className="text-6xl sm:text-8xl font-serif text-white italic leading-tight">
              {INVITATION_DATA.groom} <br />
              <span className="text-4xl sm:text-6xl not-italic font-accent opacity-80">&</span> <br />
          ...
          >
            ...
            <p className="text-white/90 font-serif text-xl sm:text-2xl tracking-widest uppercase">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 pb-24 space-y-32">
        ...
        <section className="space-y-12">
          <div className="text-center space-y-4">
            <p className="text-[10px] uppercase tracking-[0.4em] text-primary font-bold opacity-60">Moments Captured</p>
            <h2 className="text-4xl sm:text-5xl font-serif text-primary italic">Our Gallery</h2>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {INVITATION_DATA.gallery.map((img, index) => (
              <motion.div
                ...
                className={`relative rounded-2xl overflow-hidden shadow-lg ${index % 3 === 0 ? 'col-span-2 aspect-video' : 'aspect-[3/4]'}`}
              >
                <img 
                  src={img} 
                  alt={`Gallery ${index}`} 
        ...
        <section className="text-center space-y-12 py-12">
          ...
          >
            ...
            <div className="space-y-4">
              <h2 className="text-3xl font-serif text-primary italic">Will You Join Us?</h2>
              <p className="text-text/60 text-sm max-w-xs mx-auto">
                Please let us know if you can make it by December 1st, 2026. We can't wait to celebrate with you!
              </p>
            </div>
            <a
              href={INVITATION_DATA.rsvpUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center px-12 py-4 rounded-full bg-primary text-white font-medium tracking-widest uppercase text-xs hover:bg-primary/90 transition-all shadow-xl shadow-primary/20 hover:scale-105"
            >
              RSVP Now
            </a>
          </motion.div>
        ...
        <section className="space-y-12">
          ...
          <form className="glass-card p-8 space-y-6" onSubmit={(e) => e.preventDefault()}>
            ...
            <button className="w-full py-4 rounded-xl bg-primary text-white font-medium tracking-widest uppercase text-xs hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">
              Send Wishes
            </button>
          </form>
        </section>
        ...
        <footer className="text-center space-y-8 pt-12">
          ...
          <p className="text-[10px] uppercase tracking-widest text-text/30 pt-12">
            Made with Love for Meenaya & Kavya
          </p>
        </footer>
